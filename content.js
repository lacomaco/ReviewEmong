...
document.addEventListener("mousedown", (event) => {
  ...
  document.addEventListener('DOMNodeInserted',(event)=>{
    ...
    if(element.classList && element.classList.contains('js-line-comments') && element.getAttribute('colspan')==='2'){
      const buttonContainer = element.querySelector('.form-actions');

      const reviewButton = document.createElement('div');
      
      'btn float-md-right float-none width-full width-md-auto'.split(' ').forEach((cssClass)=>{
...
async function sendGPTReview(code,comments){
  ...
  const payload = {
    ...
    messages: [
      {
        "role":'system',
        "content": "Hello, ChatGPT. From now on you are going to act as ReviewEmong, an experienced software engineer who excels at building web applications. Your goal is to help review code as a software engineer. Find something good in follow code. And if there are Code that can be improved find them too.",
      }, {
        "role":'user',
        "content":`${code}\n${language.korean}`,
      }
  ...
  const response = await callOpenAiStream(payload,comments.querySelector('textarea'));
}

async function callOpenAiStream(payload,textArea) {

  chrome.runtime.sendMessage({type:'get-api-key'},async ({OPEN_AI_KEY})=>{
    if(!OPEN_AI_KEY){
  ...
  reader.read().then(function processText({done,value}){
    ...
    const datas = value.split("data:").map((data)=>{
      
      return data.trim()
    }).filter((data)=> isJSON(data)).map((data)=>JSON.parse(data)).reduce((acc,curr)=> {
      if(curr.choices[0].delta.content){
        return acc+curr.choices[0].delta.content;
      }
      return acc;
      acc+curr.choices[0].delta.content
    },'');
